...
def stat_report(stats, res_param='v', subs=['max','eq'], spec=None, specLabels=None, fraction='1/3', init_rows=3, init_cols=1, name=f'stat_results', width=20):    
    ...
    spec : list
        Contains dicionaries with characteristics to be written in the initial columns, e.g. [train_daytime, train_time]
    specLabels : list
    ...
    name : str, optional
        '', by default f'{project_name}_stat_results'
    width : int, optional
    ...
    split = 1
    width = len(F)*n_stats + init_cols
    while width > 20:   # 20 is a reasonable number of columns to fit on a page
        split += 1
        width -= 20 + init_cols
...
def trains_report(stats, res_param, params, train_daytime, fraction='1/3', init_rows=3-1, init_cols=3, name='trains'):
